...
export default function Templates() {
  ...
  return (
    <section className="bg-gray-50 py-12">
      <Heading
        title="Pick a Template for Your Website"
        description="Browse beautiful templates and select one to kickstart your website. Hover on any template to preview images."
      />
          ...
          {templatesData.map((item) => (
            <SwiperSlide key={item.id}>
              <div
                ...
                className={`cursor-pointer bg-white rounded-3xl shadow-md hover:shadow-2xl transition-all duration-500 overflow-hidden border-2 ${
                  selectedTemplate === String(item.id) ? "border-primary" : "border-transparent"
                }`}
              >
                {/* Image Carousel */}
                <div className="relative h-64 group">
                  ...
                  {/* Hover Overlay */}
                  <div className="absolute inset-0 bg-gradient-to-t from-black/20 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 rounded-t-3xl"></div>
                </div>
